...
Node *sort_push(Node *tmp_list, int num){
    ...
    if (num < tmp_list->data) {
        tmp->next = tmp_list;
        return tmp;
    }

    Node *work_list=tmp_list;
    while (work_list->next != NULL && work_list->next->data < num){
        work_list=work_list->next;
    }
    tmp->next=work_list->next;
    work_list->next=tmp;
...
int main(int argc, char **argv){
    Node *list=NULL;
    list=sort_push(list, 5);
    list=sort_push(list, 15);
    list=sort_push(list, 10);
    printf("\n");
    print_list(list);
    search(list, 15);
    free_structs(list);
